// ============================================================
//  app.js — Main controller v4.2
//  Fix: handleUnlock blindado con try/catch; log de errores
//       visible en consola para diagnóstico.
//  v4.2: bootApp ahora también dispara fetchAllBscScanData()
//        (datos on-chain Etherscan V2 / BscScan), en paralelo
//        al flujo existente, sin bloquear nada.
// ============================================================
...
function bootApp() {
  ...
  // Datos on-chain (BscScan/Etherscan V2) — corre aparte del polling de
  // precios, una sola vez por token (con cache de 6h en localStorage),
  // no bloquea ni altera el flujo de precios/alertas existente.
  setTimeout(() => {
    if (typeof fetchAllBscScanData === 'function') {
      fetchAllBscScanData().catch(err =>
        console.warn('[bscscan] fetchAllBscScanData error:', err)
      );
    }
  }, 3000);
}
