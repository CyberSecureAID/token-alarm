...
/**
 * ROUTE A — proveedor primario (embed directo, BSC)
 */
...
/**
 * ROUTE B — proveedor secundario (iframe embed, BSC)
 * PancakeSwap soporta embed via query params en /swap
 */
...
/**
 * ROUTE C — proveedor terciario (iframe embed, ETH/multi-chain)
 * Uniswap soporta embed via query params
 */
...
function swapUrlFor(address, provider) {
  switch (provider) {
    ...
    default:  return swapUrlRouteA(address);   // 'a' o undefined → ROUTE A
  }
}
...
// ============================================================
const _swapProvider = {};   // address → 'a' | 'b' | 'c'
const _swapLoaded   = new Set();

/**
 * Devuelve el proveedor activo para una card.
 * Por defecto ROUTE A.
 */
...
/**
 * Cambia el proveedor activo y recarga el iframe.
 * Llamado desde los botones del selector dentro del panel swap.
 */
function switchSwapProvider(address, provider) {
  ...
  // Recargar iframe con el nuevo proveedor
...
/**
 * Carga inicial del panel swap (lazy).
 * Si ya fue cargado, solo actualiza el iframe al proveedor activo.
 */
function _loadSwap(address) {
  const provider = getSwapProvider(address);

  if (_swapLoaded.has(address)) {
    // Panel ya montado — solo refresca iframe si el proveedor cambió
    const wrap = document.getElementById('swap-iframe-wrap-' + address);
...
function buildTokenCard(token, state) {
  ...
  // ── Selector de proveedores swap (nombres propios, sin marca externa) ──
  ...
  card.innerHTML = `
    ...
    <!-- PANEL: SWAP v5.8 — selector de 3 rutas, nombres propios -->
    <div class="chart-panel swap-panel" id="swap-${token.address}" style="display:none">

      <!-- Header con selector de proveedor y badge de red -->
      ...
      <!-- Selector de rutas — solo visible dentro del panel swap -->
      ...
      <div class="swap-iframe-wrap" id="swap-iframe-wrap-${token.address}">
        ...
        <!-- v5.7: overlay eliminado — MetaMask visible -->
