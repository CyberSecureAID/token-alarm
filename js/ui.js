...
// ============================================================
//  ON-CHAIN INFO (BscScan / Etherscan V2) — v1.1
//  Solo se muestra lo que el plan free puede entregar:
//  estado de verificación del contrato. El resto de campos
//  (creador, fecha creación, balance BNB, proxy) requieren
//  endpoints Pro o no responden de forma confiable y se
//  eliminaron de la interfaz. Ver bscscan.js.
// ============================================================
      ...
      <div class="card-stats-onchain-grid">
        <div class="stat-item">
          <div class="stat-label-row">
            ...
            <button class="info-btn" data-info-key="verified" data-info-address="${address}" title="Más info" aria-label="Más información">+<span class="info-btn-text">info</span></button>
          </div>
...
function buildTokenCard(token, state) {
  ...
  card.innerHTML = `
    ...
    <div class="card-stats">
      <div class="stat-item">
        <div class="stat-label-row">
          ...
          <button class="info-btn" data-info-key="volume24h" data-info-address="${token.address}" title="Más info" aria-label="Más información">+<span class="info-btn-text">info</span></button>
        </div>
      ...
      <div class="stat-item">
        <div class="stat-label-row">
          ...
          <button class="info-btn" data-info-key="liquidity" data-info-address="${token.address}" title="Más info" aria-label="Más información">+<span class="info-btn-text">info</span></button>
        </div>
      ...
      <div class="stat-item">
        <div class="stat-label-row">
          ...
          <button class="info-btn" data-info-key="marketCap" data-info-address="${token.address}" title="Más info" aria-label="Más información">+<span class="info-btn-text">info</span></button>
        </div>
      ...
      <div class="stat-item">
        <div class="stat-label-row">
          ...
          <button class="info-btn" data-info-key="fdv" data-info-address="${token.address}" title="Más info" aria-label="Más información">+<span class="info-btn-text">info</span></button>
        </div>
      ...
      <div class="stat-item">
        <div class="stat-label-row">
          ...
          <button class="info-btn" data-info-key="txns24h" data-info-address="${token.address}" title="Más info" aria-label="Más información">+<span class="info-btn-text">info</span></button>
        </div>
    ...
    <div class="card-stats-extra">
      <div class="stat-item">
        <div class="stat-label-row">
          <span class="stat-label">SUPPLY TOTAL</span>
          <button class="info-btn" data-info-key="totalSupply" data-info-address="${token.address}" title="Más info" aria-label="Más información">+<span class="info-btn-text">info</span></button>
        </div>
        <div class="stat-value" id="stat-supply-${token.address}">${formatSupply(state.totalSupply)}</div>
      </div>
      <div class="stat-item">
        <div class="stat-label-row">
          <span class="stat-label">PAR DESCUBIERTO</span>
          <button class="info-btn" data-info-key="pairAddress" data-info-address="${token.address}" title="Más info" aria-label="Más información">+<span class="info-btn-text">info</span></button>
        </div>
...
// ============================================================
//  POPUPS INFORMATIVOS ("+ info") — v1.0
//  Botones discretos en cada stat de la card. Al hacer click
//  abren una ventana emergente con la explicación correspondiente.
//  La liquidez se muestra en palabras (español) únicamente
//  dentro del popup — en la card sigue en formato numérico.
// ============================================================
const INFO_TEXTS = {
  volume24h: {
    title: 'Volumen 24H',
    body: (state) => `
      <p>Es la suma en dólares de todas las compras y ventas de este token realizadas en las últimas 24 horas, en el par de mayor liquidez detectado.</p>
      <p>Un volumen alto indica que el token se está negociando activamente. Un volumen muy bajo puede significar poco interés o poca liquidez disponible para operar sin afectar el precio.</p>
    `,
  },
  liquidity: {
    title: 'Liquidez',
  ...
  marketCap: {
    title: 'Market Cap (circulante)',
    body: () => `
      <p>El Market Cap o "capitalización de mercado" es el valor total del token actualmente en circulación. Se calcula multiplicando el precio actual por la cantidad de tokens en circulación (no el supply total, sino el que realmente está en manos de la gente y disponible para operar).</p>
      <p>Sirve para comparar el "tamaño" de un token frente a otros, más allá de su precio unitario.</p>
    `,
  },
  fdv: {
    title: 'FDV (Fully Diluted Valuation)',
    body: () => `
      <p>El FDV es el valor total que tendría el token si TODO el supply (incluyendo tokens que aún no están en circulación) estuviera ya en el mercado al precio actual. Se calcula multiplicando el precio actual por el supply total.</p>
      <p>Si el FDV es mucho más alto que el Market Cap, significa que todavía falta liberar una gran cantidad de tokens al mercado, lo cual puede generar presión de venta a futuro.</p>
    `,
  },
  txns24h: {
    title: 'Transacciones 24H',
    body: () => `
      <p>Es el número total de operaciones de compra y venta de este token registradas en las últimas 24 horas, en el par principal detectado.</p>
      <p>Una cantidad alta de transacciones suele indicar interés activo de la comunidad y mayor actividad de mercado, más allá del monto total negociado (volumen).</p>
    `,
  },
  totalSupply: {
    title: 'Supply Total',
    body: () => `
      <p>El Supply Total es la cantidad total de unidades del token que existen, ya sea que estén en circulación, bloqueadas, quemadas o reservadas.</p>
      <p>Este dato, combinado con el precio actual, permite calcular el FDV (valor totalmente diluido) del token.</p>
    `,
  },
  pairAddress: {
    title: 'Par Descubierto',
    body: (state, token) => {
      const pairAddr = state?.pairAddress || token?.pairAddress;
      return `
        <p>Es la dirección del contrato del par de liquidez (pool) que el sistema encontró automáticamente para este token, usado para obtener el precio, volumen y liquidez en tiempo real.</p>
        <p>Este par se descubre y guarda automáticamente la primera vez que se consulta el token, priorizando el pool con mayor liquidez disponible en BSC.</p>
        ${pairAddr ? `
          <div class="info-pair-row">
            <span class="info-pair-addr">${pairAddr}</span>
            <button class="info-copy-btn" data-copy-value="${pairAddr}">⎘ COPIAR</button>
          </div>
        ` : '<p><em>Todavía no se descubrió un par para este token.</em></p>'}
      `;
    },
  },
  verified: {
    ...
    body: () => `
      <p>Indica si el código fuente del contrato del token fue verificado públicamente en BscScan (el explorador oficial de BNB Smart Chain), usando la API de Etherscan V2.</p>
      <p>Que un contrato esté <strong>verificado</strong> significa que su código fue publicado y es legible por cualquier persona, lo que permite auditar su funcionamiento. No es garantía de que el token sea seguro o legítimo, pero sí aporta transparencia.</p>
      <p>Si aparece como <strong>no verificado</strong>, el código no está disponible públicamente, lo cual no implica necesariamente un problema, pero sí reduce la posibilidad de auditarlo antes de operar.</p>
    `,
  },
};
...
function showInfoPopup(key, address) {
  ...
  if (!info) return;
  const state = address ? priceState[address] : null;
  const token = address ? getToken(address) : null;

  let modal = document.getElementById('info-popup-overlay');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'info-popup-overlay';
    modal.className = 'info-popup-overlay';
    document.body.appendChild(modal);
    modal.addEventListener('click', e => {
      if (e.target === modal) closeInfoPopup();
    });
  }
  ...
  modal.innerHTML = `
    <div class="info-popup-box">
      <div class="info-popup-header">
        <span class="info-popup-title">${info.title}</span>
        <button class="info-popup-close" id="info-popup-close-btn" title="Cerrar" aria-label="Cerrar">✕</button>
      </div>
      <div class="info-popup-body">
        ${info.body(state, token)}
      </div>
    </div>
  `;
  ...
  modal.querySelector('.info-copy-btn')?.addEventListener('click', e => {
    const val = e.target.dataset.copyValue;
    if (val) copyAddress(val);
  });
}

function closeInfoPopup() {
  const modal = document.getElementById('info-popup-overlay');
...
// Delegación global de eventos para todos los botones "+ info"
