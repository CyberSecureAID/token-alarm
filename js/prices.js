  ...
  if (tokensWithoutPair.length > 0) {
    for (const token of tokensWithoutPair) {
      try {
        ...
        // El endpoint /tokens/v1/bsc/<address> devuelve:
        // - Array directo de pares si hay resultados
        // - Objeto con propiedad 'pairs' si hay resultados
        // - Array vacío o null si no hay resultados
        let pairs = [];
        ...
        if (pairs.length === 0) {
          console.warn(`[prices] Sin pares para ${contractTag(token.address)} en DexScreener - intentando GeckoTerminal`);
          // No marcar como error todavía, dejar que GeckoTerminal intente
          continue;
        }
...
async function fetchViaGecko() {
  ...
  for (const token of TOKENS) {
    ...
    if (!tData) {
      // GeckoTerminal no tiene datos para este token
      // Mantener el estado actual (podría tener datos de DexScreener)
      continue;
    }
